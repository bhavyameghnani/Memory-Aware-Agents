...
def setup_oracle_database(
    ...
    try:
        admin_conn = oracledb.connect(
            ...
            dsn=dsn,
            mode=oracledb.SYSDBA if admin_user.lower() == "sys" else None
        )
    ...
    try:
        ...
        with admin_conn.cursor() as cur:
            ...
            cur.execute(f"""
                ...
                BEGIN
                    ...
                    IF user_count = 0 THEN
                        EXECUTE IMMEDIATE
                            'CREATE USER VECTOR IDENTIFIED BY "{vector_password}"{ts_clause}';

                        EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE TO VECTOR';
                        EXECUTE IMMEDIATE 'GRANT CREATE SESSION TO VECTOR';
            ...
            # Always ensure tablespace is set
            if assm_ts:
...
def connect_to_oracle(max_retries=3, retry_delay=5, user="None", password="None",
                      dsn="None", program="None"):
    """
...
# llm = GeminiLLM(client, model="gemini-1.5-flash")

# toolbox = Toolbox(memory_manager, llm, embedding_model)
        
class Toolbox:
    def __init__(self, memory_manager, llm, embedding_function):
    ...
    def _augment_docstring(self, docstring: str, source_code: str = "") -> str:
        ...
        prompt = (
            ...
            f"{code_section}\n\n"
            "Return ONLY the improved docstring as plain text."
        )
...
that a user might ask when they need this tool.
...
"""
        ...
        try:
            queries = json_lib.loads(text)
            if isinstance(queries, list):
                return queries
            return []
        except Exception:
            # fallback (Gemini sometimes adds text)
            return [text]
    ...
    def register_tool(self, func: Optional[Callable] = None, augment: bool = False):
        ...
        def decorator(f: Callable):
            ...
            if self._tool_exists_in_db(tool_name):
                ...
                print(f"⏭️ Tool '{tool_name}' already exists")
                return tool_name
            ...
            if augment:
                ...
                augmented_docstring = self._augment_docstring(docstring, source_code)
...
Conversation:
{cleaned[:6000]}"""
        response = llm_client.generate_content(retry_prompt)
        summary = response.text
...
Summary:
{summary}"""
    response = llm_client.generate_content(desc_prompt)
    desc_response = response.text
